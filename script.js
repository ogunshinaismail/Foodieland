...
let recipeArr = slice.map( recipe => {
...
recipesCard.innerHTML = recipeArr


const tryNewSize = 8
const tryNewSlice = recipes.slice(0, tryNewSize)

let tryNewArr = tryNewSlice.map( recipe => {
    return `<div class="new-recipes">
                <i class="fa-solid fa-heart like"></i>
                <img src="${recipe.img}" alt="">
                <p class="recipe-desc">${recipe.name}</p>
...
tryNewArr = tryNewArr.join('');
tryNew.innerHTML = tryNewArr
